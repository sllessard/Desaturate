...
let pageInit = (function () {
  ...
  function titleCreate() {
    ...
    if(pageTitle !== ""){
      document.title = siteTitle + " \xBB " + pageTitle;
    } else {
  ...
  function loadedPage() {
    ...
    //Adds class to highlight active page in nav
...
let modal = (function() {
  ...
  //Create array of other modal items in restricted section for pagination
  function modalArrayBuilder($that, itemsContainer, item, dataRef, lazyRef) {
    modalArray = $that.closest(itemsContainer).find(item).map(function(index){
      if($(this).attr(lazyRef) !== undefined) {
        if ($that.attr(lazyRef) == $(this).attr(lazyRef)) {
          displayedIndex = index;
        }
        return $(this).attr(lazyRef);
      } else { 
        if ($that.attr(dataRef) == $(this).attr(dataRef)) {
          displayedIndex = index;
        }
  ...
  function modalImageDisplay() {
    ...
    //If image is larger than window do not center
    if($("#imageWrapper").innerHeight() > windowHeight) {
  ...
  function modalPag($that) {
    if ($that.hasClass("leftImagePag")) {
      displayedIndex -= 1;
      if (displayedIndex < 0) {
        displayedIndex = modalArray.length -1;
      }
    } else {
      displayedIndex += 1;
      if (displayedIndex > (modalArray.length - 1)) {
        displayedIndex = 0;
      } 
    }
    indexPosition();
